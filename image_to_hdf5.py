...
class ImageToHDF5:
  ...
  def __init__(self,delta=1.0,resize=True):
    ...
    self.h5f = h5py.File('dataset_h5/images_640x480_delta_{}.h5'.format(delta),'w')
    self.delta = delta
  ...
  def prepare_train(self, limit=None):
    ...
    data = self.h5f.create_dataset("data_train",(periodNum,channel,width,height), dtype='float32')
