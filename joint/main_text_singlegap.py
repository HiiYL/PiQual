...
def create_googlenet(embedding_layer,weights_path=None, heatmap=False):
    ...
    googlenet = Model(input=[input, comment_input], output=output_aesthetics)
...
model = create_googlenet(embedding_layer, 'googlenet_aesthetics_weights.h5', heatmap=False)
...
checkpointer = ModelCheckpoint(filepath="googlenet_aesthetics_singlegap_weights{}.h5".format(time_now), verbose=1, save_best_only=True)

reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1,patience=3)

csv_logger = CSVLogger('training_singlegap_aesthetics{}.log'.format(time_now))


# class_weight = {0 : 0.67, 1: 0.33}
model.fit([X_train, X_train_text],Y_train,nb_epoch=20, batch_size=32, shuffle="batch",
 validation_data=([X_test,X_test_text], Y_test), callbacks=[csv_logger,checkpointer])#,reduce_lr])#,class_weight = class_weight)

from keras.utils.visualize_util import plot
plot(model, to_file='model_singlegap.png')
